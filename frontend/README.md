...
6. Search + Replace all instances of 'tale-of-token-repo' with your project name
7. Search + Replace 'soleano' with your company name.  This is the NX project scope that allows you to import libraries using @soleano

That's it for the renaming!  Now to test...

```npm install```

```npx nx run tale-of-token:serve``` (where tale-of-token is the name you replaced with above)
...
Libraries are shareable across libraries and applications. They can be imported from `@tale-of-token-repo/mylib`.

## Development server

Run `npx nx run tale-of-token:serve` for a dev server. Navigate to http://localhost:4200/. The app will automatically reload if you change any of the source files.
